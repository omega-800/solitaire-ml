...
type card_val = N2 | N3 | N4 | N5 | N6 | N7 | N8 | N9 | N10 | Jack | Queen | King | Ace [@@deriving enum, show]
...
let show_card_val (cval : card_val) : string = 
  ...
  | N10 -> "10" | N9 -> "9" | N8 -> "8" | N7 -> "7"
  | N6 -> "6" | N5 -> "5" | N4 -> "4" | N3 -> "3"
  | N2 -> "2"

let show_card_type (ctype : card_type) : string = 
  match ctype with
  | Diamond -> Color.red "D" | Heart -> Color.red "H"
  | Spade -> Color.blue "S" | Club -> Color.blue "C"
